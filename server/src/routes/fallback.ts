...
fallbackRouter.get('/', async (_req: Request, res: Response) => {
  ...
  const keyCounts = await db.all<{ platform: string; count: number }>(`
...
fallbackRouter.post('/sort/:preset', async (req: Request, res: Response) => {
  ...
  const db = getDb();
  const models = await db.all(`SELECT m.id FROM models m ORDER BY ${orderBy}`) as { id: number }[];

  await db.transaction(async (tx) => {
    for (let i = 0; i < models.length; i++) {
      await tx.run('UPDATE fallback_config SET priority = ? WHERE model_db_id = ?', [i + 1, models[i].id]);
    }
  });
...
fallbackRouter.get('/token-usage', async (_req: Request, res: Response) => {
  ...
  const models = await db.all(`
    ...
    WHERE m.enabled = 1
    ORDER BY fc.priority ASC
  `) as { platform: string; model_id: string; display_name: string; monthly_token_budget: string; priority: number }[];
