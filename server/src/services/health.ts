...
export async function checkKeyHealth(keyId: number): Promise<KeyStatus> {
  const db = getDb();
  const row = await db.get('SELECT * FROM api_keys WHERE id = ?', [keyId]) as any;
  if (!row) return 'error';
  ...
  try {
    ...
    await db.run("UPDATE api_keys SET status = ?, last_checked_at = datetime('now') WHERE id = ?", [status, keyId]);
    ...
    await db.run("UPDATE api_keys SET status = ?, last_checked_at = datetime('now') WHERE id = ?", ['error', keyId]);
    return 'error';
  }
}
...
export async function checkAllKeys(): Promise<void> {
  const db = getDb();
  const keys = await db.all('SELECT id, platform FROM api_keys WHERE enabled = 1') as { id: number; platform: string }[];
